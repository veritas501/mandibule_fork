...
#define FIX_SP_JMP(stack, addr)                \
    asm volatile("xchg %%rsp, %0; jmp *%%rax;" \
                 : "=r"(stack)                 \
                 : "0"(stack), "a"(addr));
...
#define FIX_SP_JMP(stackp, addrp)                                             \
    asm volatile("mov x0, sp; mov x1, %[stack]; sub x0, x1, x0; add sp, sp, " \
                 "x0; br %[addr]"                                             \
                 :                                                            \
                 : [stack] "r"(stackp), [addr] "r"(addrp)                     \
                 : "x0", "x1");

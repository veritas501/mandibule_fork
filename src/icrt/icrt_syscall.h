...
// we then use this macro to define macros for syscalls with specifid number of
// args finally, we use those macros to define the functions that are of
// interest to us
...
_syscall0(SYS_getpid, sys_getpid, int);

_syscall1(SYS_exit, sys_exit, int, int);
// void sys_exit(int c) { _exit(c); }

_syscall1(SYS_close, sys_close, int, int);
_syscall1(SYS_brk, sys_brk, long, unsigned long);

_syscall2(SYS_munmap, sys_munmap, long, char *, int);
;
_syscall3(SYS_read, sys_read, long, int, void *, size_t);
_syscall3(SYS_write, sys_write, long, int, const void *, size_t);
_syscall3(SYS_lseek, sys_lseek, long, int, long, int);
_syscall3(SYS_mprotect, sys_mprotect, long, void *, long, int);

#if __i386__ || __arm__ || __x86_64__
_syscall3(SYS_open, sys_open, int, char *, int, int);
#else
_syscall4(SYS_openat, sys_openat, int, int, char *, int, int);
#define AT_FDCWD -100
#define sys_open(a, b, c) sys_openat(AT_FDCWD, a, b, c)
#endif

_syscall4(SYS_ptrace, sys_ptrace, long, int, int, void *, void *);
_syscall4(SYS_wait4, sys_wait4, int, int, int *, int, void *);

#if __i386__ || __arm__
_syscall6(SYS_mmap2, sys_mmap, void *, void *, long, int, int, int, long);
#else
_syscall6(SYS_mmap, sys_mmap, void *, void *, long, int, int, int, long);
#endif
